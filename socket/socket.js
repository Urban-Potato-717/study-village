...
// 본격적으로 들어가기 전 좌석/사용자/캐릭터를 한 번에 가져오기 위한 공통 SELECT
...
module.exports = function (io) {          // bin/www에서 io 받음 (소켓 서버 받기)
  io.on('connection', function (socket) { // 누군가 소켓 연결 시 마다 실행
    ...
    socket.on('joinRoom', function (payload) {
      ...
      socket.join('room-' + roomId); //! 핵심 ex) room - 3로 들어감
      socket.data.roomId = roomId;   //이 Socket에 roomId를 기억하게
    ...
    // 라우터(routes/study.js)에서 직접 broadcast 하므로 호환용으로만 유지 (거의 사용하지 않음)
    ...
    socket.on('disconnect', async function () {
      ...
      try {
        ...
        if (!seat) return;
        // F5시 즉시 좌석 해제 - DB에서 좌석 점유 상태를 삭제
        await pool.query(
